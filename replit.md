...
- **Advanced Reporting Module** (Admin-only):
  ...
  - **FoodStation Consumption Control Report** (`/foodstation-consumption-control`): Detailed consumption tracking with user filtering (all users or individual), date range selection, KPIs (total items, total value, period coverage, record count), and comprehensive table showing matricula, user name, product details, quantities, unit/total prices, and consumption timestamps. Includes monthly totals per user and auto-refresh every 30 seconds.
