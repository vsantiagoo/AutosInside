...
- **Data Access**: Storage abstraction layer (`IStorage`), synchronous SQLite operations, type-safe queries with TypeScript interfaces, Zod for schema validation. New storage methods: getProductsBySector, getSectorPerformanceIndicators, getStockTransactionsBySector for sector-specific operations. Inventory valuation uses `unit_price` (acquisition cost) per GAAP/IFRS accounting standards, documented in storage functions.
...
- **Financial Data Model**: System distinguishes between `unit_price` (acquisition/purchase cost from supplier) and `sale_price` (selling price/revenue). All inventory valuations and KPIs use `unit_price` per accounting standards. `sale_price` available for revenue analysis and margin calculations.
